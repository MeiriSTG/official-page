...
const SCORES = [
  ...
  {
    "結界チーム(FinalA)": [8, 14, 9, 14],
    "結界チーム(FinalB)": [8, 14, 9, 18],
    "詠唱チーム(FinalA)": [8, 14, 9, 15],
    "詠唱チーム(FinalB)": [8, 14, 9, 18],
    "紅魔チーム(FinalA)": [9, 15, 10, 16],
    "紅魔チーム(FinalB)": [9, 15, 10, 20],
    "幽冥チーム(FinalA)": [9, 15, 9, 16],
    "幽冥チーム(FinalB)": [9, 15, 9, 20],
    "霊夢(FinalA)": [12, 19, 14, 42],
    "霊夢(FinalB)": [12, 19, 14, 72],
    "紫(FinalA)": [9, 15, 10, 18],
    "紫(FinalB)": [9, 15, 10, 23],
    "魔理沙(FinalA)": [11, 18, 12, 29],
    "魔理沙(FinalB)": [11, 18, 12, 38],
    "アリス(FinalA)": [13, 20, 13, 64],
    "アリス(FinalB)": [13, 20, 13, 76],
    "咲夜(FinalA)": [14, 21, 14, 64],
    "咲夜(FinalB)": [14, 21, 14, 84],
    "レミリア(FinalA)": [9, 15, 11, 20],
    "レミリア(FinalB)": [9, 15, 11, 26],
    "妖夢(FinalA)": [9, 15, 10, 16],
    "妖夢(FinalB)": [9, 15, 10, 20],
    "幽々子(FinalA)": [10, 16, 11, 50],
    "幽々子(FinalB)": [10, 16, 11, 57]
  },
  {
    "霊夢A": [10, 16, 10, 10],
    "霊夢B": [10, 16, 10, 9],
...
function init() {
  ...
  for (let i = 0; i < SCORES.length; ++i) {
    const ths = [TITLES[i], 'Easy,Normal', 'Hard,Lunatic', 'Extra,Phantasm', 'NB'];
    const tdss = [];
    ...
    const widths = Array.from(trs[0].getElementsByTagName('th')).map(th => th.offsetWidth);
    const max_width = Math.max(...widths);
    for (const n of trs) {
      for (const m of n.cells) {
        m.style.width = max_width + "px";
      }
    }
  }
